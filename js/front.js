...
function signform() {
	...
	if (username.length < 3) {
		...
		$("#Users_username_em_").show();
		$('#Users_username_em_').text(
				Yii.t('admin', 'Username should have minimum 3 characters'));
		$('#Users_username').keydown(function() {
			$('#Users_username_em_').hide();
		});
		return false;
...
function mobile_verification() {
	...
	if(mobile == '' && counrty_code == '')
	{
		...
		$('.mobile-error').html('Enter your country code,mobile number');
		//$('.counrty-error').html(' ');
		return false;
	...
	if(counrty_code == '' && mobile != '') {
		$('.mobile-error').css('display','inline-block');
		$('.mobile-error').html('Enter country code');
		//$('.mobile-error').html(' ');
		return false;
	}
	...
	$.ajax({
		...
		success : function(data) {
			...
			{	
				...
				$('.rand_code').html("Mobile verification code successfully send.");
			...
			{	
				...
				$('.rand_code').html("message sending failed.");
...
function select_shipping(shippingId)
{
	...
	$(".kader-acc-addr-cnt").css("box-shadow","none");
